...
def article_snapshot(documentation_root):
    snapshot = {}
    if not documentation_root.is_dir():
        return snapshot
    for path in documentation_root.rglob("*.json"):
...
def render_dashboard(comparisons, arguments, sources=None):
    sources = sources or ["api"]
    base = arguments.hosting_base_path.rstrip("/")
    data = json.dumps(
        {"comparisons": comparisons, "sources": sources}, separators=(",", ":")
    ).replace("</", "<\\/")
    comparison_options = "\n".join(
    ...
    if sources == ["api"]:
        introduction = "Generated from immutable public symbol-graph snapshots. Cached release documentation does not need to be rebuilt to render this comparison."
    elif sources == ["article"]:
        introduction = "Generated from immutable DocC article render nodes. Source-service metadata and linked-page references are excluded to avoid unrelated changes."
    else:
        introduction = "Generated from immutable public symbol graphs and DocC article render nodes. Cached release documentation does not need to be rebuilt to render this comparison."
...
<head>
  ...
  <title>{changes_title} | {html.escape(arguments.project_name)} Documentation</title>
  <style>
...
<body>
  ...
  <script>
    ...
    function visibleChanges(comparison) {{ const query=search.value.trim().toLowerCase(); return comparison.changes.filter(change=>{{ const item=entity(change), searchable=(item.displayId+' '+item.title+' '+(item.declaration||'')+' '+(item.preview||'')).toLowerCase(); return (sourceFilter.value==='all'||change.source===sourceFilter.value)&&(filter.value==='all'||change.type===filter.value)&&(!query||searchable.includes(query)); }}); }}
    ...
    function card(change,comparison) {{ const item=entity(change), linkVersion=change.type==='removed'?comparison.previousVersion:comparison.currentVersion, href=item.path?BASE+'/'+linkVersion+item.path:null, title=href?`<h2><a href="${{escapeHTML(href)}}">${{escapeHTML(item.title)}}</a></h2>`:`<h2>${{escapeHTML(item.title)}}</h2>`, documentationLink=href?`<footer><a href="${{escapeHTML(href)}}">View documentation →</a></footer>`:''; let details=''; if(change.source==='article') details=articleDiff(change); else if(change.type==='modified') details=`<div class="declaration"><span>Current · ${{escapeHTML(comparison.currentVersion)}}</span><code>+ ${{escapeHTML(change.current.declaration)}}</code></div><div class="declaration"><span>Previous · ${{escapeHTML(comparison.previousVersion)}}</span><code>− ${{escapeHTML(change.previous.declaration)}}</code></div>`; else details=`<div class="declaration"><span>${{escapeHTML(linkVersion)}}</span><code>${{escapeHTML(item.declaration)}}</code></div>`; const availability=item.availability?.length?`<p class="availability">${{escapeHTML(item.availability.join(' · '))}}</p>`:''; return `<article class="change-card ${{change.type}}"><header><span class="badge">${{change.type[0].toUpperCase()+change.type.slice(1)}}</span><span class="kind">${{escapeHTML(item.kind)}}</span></header>${{title}}<p class="symbol-path">${{escapeHTML(item.displayId)}}</p>${{availability}}${{details}}${{documentationLink}}</article>`; }}
    ...
    function render() {{ const comparison=activeComparison(), changes=visibleChanges(comparison), pages=Math.max(1,Math.ceil(changes.length/PAGE_SIZE)); page=Math.min(page,pages-1); const start=page*PAGE_SIZE, shown=changes.slice(start,start+PAGE_SIZE), apiCount=comparison.changes.filter(change=>change.source==='api').length, articleCount=comparison.changes.filter(change=>change.source==='article').length, notes=[]; if(apiCount) notes.push(apiCount.toLocaleString()+' public API'); if(articleCount) notes.push(articleCount.toLocaleString()+' article'); document.getElementById('title').innerHTML=escapeHTML(comparison.previousVersion)+' <span>→</span> '+escapeHTML(comparison.currentVersion); document.getElementById('note').textContent=(notes.join(' · ')||'No')+' changes'; for(const type of ['added','modified','removed']) document.getElementById(type).textContent=comparison.counts[type].toLocaleString(); document.getElementById('result-count').textContent=changes.length?`Showing ${{start+1}}–${{start+shown.length}} of ${{changes.length.toLocaleString()}} changes`:'No matching changes'; document.getElementById('previous').disabled=page===0; document.getElementById('next').disabled=page>=pages-1; list.innerHTML=shown.map(change=>card(change,comparison)).join(''); document.getElementById('empty').hidden=changes.length!==0; persistState(); }}
...
def main():
    ...
    for version in article_versions:
        ...
        article_snapshots[version] = article_snapshot(article_paths[version][0])
    ...
    dashboard = render_dashboard(comparisons, arguments, change_sources)
